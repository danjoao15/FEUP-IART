...
atributo(N-G, AdjComp, _, _) --> adjetivo_super(GrAdj), adjetivo(N-G, Adj), {adj_comp(GrAdj,Adj,AdjComp)}.  
atributo(N-G, Adj, _, _) --> adjetivo(N-G, Adj).
atributo(N-G, _, [Nac|T], _) --> subst_nac(Nac,N-G), (contx_nac(N-G,T);[]).
atributo(N-G, _, _, [Cont|T]) --> subst_cont(N-G,Cont), (contx_cont(N-G,T);[]).
...
nom(N-G,_,Nom,_,_,_) --> subst(N-G,Nom).
nom(N-G,_,_,_,_,[Gen|T]) --> subst_gen(Gen,N-G), (contx_gen(N-G,T);[]).
...
query(sim) --> declarativa.
query(Lista) --> interrogativa(Lista).

			% NOVA GRAMATICA
			% consult(['~/feup-iart-master/proj1/server/gramatical.pl']).
			...
			interrogativa_livro(Lista) --> [que],
						...
						{filtrar_livros([autor=IdAutor,genero=Gen],Lista)},
						[?].
			...
			interrogativa_livro(Lista) --> [que],
                        ...
                        interrogativa_livro1(N-G),
                        procurar_autor(autor(IdAutor, _, _, _, _, _, _, _)),
                        {filtrar_livros([autor=IdAutor],Lista)},
                        [?].
		...
		interrogativa_autor(ListaFinal) --> [quais],
												art_def(_-G),
												subst(_-G,autor),
												subst_nac(Nac,_-G),
												[e],
												subst_nac(Nac1,_-G),
												[?],
												{filtrar_autores([nacionalidade=Nac],Lista),
												filtrar_autores([nacionalidade=Nac1],Lista1),
												append(Lista, Lista1, ListaFinal)	}.
      interrogativa_autor(Lista) --> [quais],
                    art_def(_-G),
                    subst(_-G,autor),
                    subst_nac(Nac,_-G),
                    [?],
...
frase_int_viver2(N, ListaFinal) -->
		locucao_seculo(Selector, Seculo),
		[e],
		locucao_seculo(Selector, Seculo2),[?],
		{autores_vivos_seculo(Selector, Seculo, Lista),
		autores_vivos_seculo(Selector, Seculo2, Lista1),
	   append(Lista, Lista1, ListaFinal)}.
frase_int_viver2(N, Lista) -->
	locucao_seculo(Selector, Seculo), [?],
	{autores_vivos_seculo(Selector, Seculo, Lista)}.
...
frase_int_nascer2(ListaFinal) -->
	locucao_ano(Selector, Ano),
	[e],
	locucao_ano(Selector, Ano2), [?],
	{filtrar_autores_nascidos(Selector, Ano, Lista),
  filtrar_autores_nascidos(Selector, Ano2, Lista1),
  append(Lista, Lista1, ListaFinal)}.
frase_int_nascer2(Lista) -->
	locucao_ano(Selector, Ano), [?],
	{filtrar_autores_nascidos(Selector, Ano, Lista)}.
...
frase_int_nascer2(ListaFinal) -->
	...
	locucao_seculo(Selector, Seculo2), [?],
	{autores_nascidos_seculo(Selector, Seculo, Lista),
	autores_nascidos_seculo(Selector, Seculo2, Lista2),
	append(Lista, Lista2, ListaFinal)
}.
...
frase_int_morrer1(N-f, Lista) -->
	frase_int_morrer2(N-f, Selector, Ano, f, Lista),
	{filtrar_autores_mortos(Selector, Ano, f, Lista)}.
frase_int_morrer1(N-m, Lista) -->
    frase_int_morrer2(N-m, Selector, Ano, m, Lista),
    {filtrar_autores_mortos(Selector, Ano, m, Lista)}.

frase_int_morrer2(N-_, Selector, Ano, Lista) -->
	verbo(N, morrer, passado),
	locucao_ano(Selector, Ano), [?].
frase_int_morrer2(N-G, =, -1,G, Lista) -->
	verbo(N, estar, presente),
